...
function makie_animation(sol; filename="two_spring_pendulum.gif", title="Animation")
    ...
    ax = GLMakie.Axis(animation[1, 1], title = GLMakie.@lift("t = $(round($time, digits = 1))"), limits=(xlimits, ylimits), aspect=1)
    ...
    GLMakie.record(animation, filename, timestamps;
...
function plot_benchmark_result(step_sizes::ProblemTypes.StepSizes, abs_errors:: ProblemTypes.BenchmarkResult; filename="benchmark_result.png")
    # Convert solution to matrix form
    step_sizes = step_sizes[2:end]
    
    # Create figure
    benchmark_plot = GLMakie.Figure()
    ax = GLMakie.Axis(benchmark_plot[1, 1], xlabel="log(step_sizes)", ylabel="log(abs_errors)", aspect = 1)
